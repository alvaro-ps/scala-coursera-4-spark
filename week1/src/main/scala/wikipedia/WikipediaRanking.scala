...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = 
    rdd
      .flatMap(article => for (lang <- langs if article.mentionsLanguage(lang)) yield (lang, article))
      .groupByKey()
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = 
    rdd
      .flatMap(article => for (lang <- langs if article.mentionsLanguage(lang)) yield (lang, 1))
      .reduceByKey(_ + _)
